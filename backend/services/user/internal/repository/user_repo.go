...
func (r *UserRepository) Create(ctx context.Context, user *model.User) error {
	query := `INSERT INTO users (email, nickname, avatar_url, password_hash, oauth_provider, oauth_id, role, language, region, status, created_at, updated_at)
			  VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'active', NOW(), NOW())
			  RETURNING id, created_at`
	return r.pool.QueryRow(ctx, query, user.Email, user.Nickname, user.AvatarURL,
		user.PasswordHash, user.OAuthProvider, user.OAuthID,
...
func (r *UserRepository) LinkOAuth(ctx context.Context, userID int64, provider, oauthID string) error {
